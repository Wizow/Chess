...
// Convertit largeur*hauteur pixels de la forme bleu, vert, rouge en entiers opacite, rouge, vert, bleu little endian (sauf blanc)
static int *BVR2ARVBm(int largeur, int hauteur, const unsigned char *donnees) {
	const unsigned char *ptrDonnees;
	unsigned char *pixels = (unsigned char*)malloc(largeur*hauteur*sizeof(int));
	unsigned char *ptrPixel;
	int index;
														...
														pixels de l'image */
	{
		...
		ptrPixel[2] = ptrDonnees[2];
		ptrPixel[3] = 0x0FF;
		if ((ptrPixel[0]==255) && (ptrPixel[1]==255) && (ptrPixel[2] == 255)){
			ptrPixel[3] = 0;
		}
		ptrDonnees += 3; /* On passe a la premiere composante du pixel suivant */
		ptrPixel += 4; /* On passe au pixel suivant */
...
void ecrisImagem(int x, int y, int largeur, int hauteur, const unsigned char *donnees) //en enlevant le fond blanc
{
	int *pixels = BVR2ARVBm(largeur, hauteur, donnees);
	ecrisImageARVB(x, y, largeur, hauteur, pixels);
	free(pixels);
}
