...
int reset_plateau(void)
{
    int i;
    int j;
                            // On remplit le tableau avec les 0 ( cases vides )
    for ( i = 2 ; i < 6 ; i++)  
    {
        for ( j = 0 ; j < 8 ; j++)
        {
...
int pion( int y, int x, int *deplacement)
{
	...
	{
		//avant 1 case
		if((strcmp(plateau[y-1][x].piece, "00") == 0) && y != 0)
		{
			deplacement[j] = y-1;
			j++;
			deplacement[j] = x;
		...
		//avant 2 cases
		if((strcmp(plateau[y-2][x].piece,"00") == 0) && y==6)
		{
			deplacement[j] = y-2;
			j++;
			deplacement[j] = x;
	...
	{
		//avant 1 case
		if((strcmp(plateau[y+1][x].piece,"00") == 0) && y+1 != 8)
		{
			deplacement[j] = y+1;
			j++;
			deplacement[j] = x;
		...
		//avant 2 cases
		if((strcmp(plateau[y+2][x].piece, "00") == 0) && y==1)
		{
			deplacement[j] = y+2;
			j++;
			deplacement[j] = x;
...
int main (void)
{
	int *deplacement = (int*)malloc(64*sizeof(int));
    reset_plateau();
   	pion(6,1,deplacement);
